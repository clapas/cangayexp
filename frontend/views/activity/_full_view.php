...
<div class="col-md-5">
  <?php 
    ...
    foreach ($activity->files as $file) {
        $items[] = ['content' => "<div class=\"img-thumbnail img-rounded\" style=\"background-image: url({$file->url})\"></div>"];
    }
    if (count($items) < 1) 
        echo '<img class="img-responsive img-thumbnail img-rounded" src="http://placehold.it/450x400" alt="">';
    else if (count($items) < 2)
        echo "<div class=\"img-thumbnail img-rounded\" style=\"background-image: url({$activity->files[0]->url})\"></div>";
